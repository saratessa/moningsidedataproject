...
df_2024<- df_2024 %>%
  mutate(across(
    c(starts_with("FALL"), starts_with("SPRING")),
    ~ as.numeric(.)
  )) %>%
 # mutate(across(matches("^(fall|spring)"), ~as.numeric(.))) %>% 
  pivot_longer(
...
#adding a column indicating subject area

df_all <- df_all %>%
  mutate(
    domain_type = case_when(
      measure %in% c("rc",  "lwid", "wa", "rf") ~ "reading",
      measure %in% c("ce", "psdi", "mf", "calc") ~ "math",
      measure %in% c("cap", "punc", "ue", "wf", "ws") ~ "writing",
      measure %in% c("lang", "comp") ~ "composite",
      TRUE ~ "other"
    )
  )
...
## Writing Partial Dataset of All Measurement Occurrences of Lowest Skill Area at Timepoint 1

```{r}

#Creating datamin dataframe which includes finding the lowest measure for a student at the beginning of year 1, then keeping all measurement occurances of that subtest going forward  

min_measure <- df_all %>%
  filter(time_cont == 1) %>%
  group_by(id) %>%
  slice_min(score, n = 1, with_ties = TRUE) %>% #this keeps multiple if a student has multuple at the same lowest score, do we want this?
  select(id, measure)

datamin <- df_all %>%
  inner_join(min_measure, by = c("id", "measure")) %>%
  arrange(id, time_cont)
...
+----------------+--------------------+---------------------------------------------------------------------------------------------+
| `student_year` | Character          | Unique identifier for each student × year (e.g., `10810_2008`).                             |
+----------------+--------------------+---------------------------------------------------------------------------------------------+
