...
datamin <- df_all %>%
  filter(time_cont==1) %>%
  group_by(id)  %>%
  mutate(min_skill = ifelse(score == min(score, na.rm = T), 1, 0)) %>%
filter(min_skill == 1)
```
...
df_below_start$measure <- as.factor(df_below_start$measure)
df_below_start$measure <- fct_relevel(df_below_start$measure, "mf")

r2 <- lm(raw_growth ~ measure + start_grade_diff_c, data = df_below_start)
summary(r2)
...
ggplot() +
  ...
  ) + coord_cartesian(ylim = c(-5, 5)) +  # <-- zoom y-axis
  theme_minimal()
...
summary_df <- df_all %>%
  filter(measure == "wf") %>%
  group_by(id) %>%
  filter(any(time_cont == 1 & grade_diff < 0)) %>%  # keep only IDs below at T1
  ungroup() %>%
  group_by(time_cont) %>%
  summarise(
    n = n(),
...
ggplot(summary_df, aes(x = time_cont, y = mean_grade_diff)) +
  geom_line() +
  geom_point(size = 2) +
  labs(
    x = "Time Point",
    y = "Mean Grade Difference",
    title = "WF Growth Over Time",
    subtitle = "Students below grade level at Timepoint 1"
  ) +
  theme_minimal()
```
