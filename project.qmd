...
Running the model for just cohort 2 and 3
...
colnames(df23)
names(df23)<- c("id", "grade", "cal_year", "measure", "score", "time")
df23 <- df23 %>%
  mutate(
    id = as.factor(id),
...
Visuals for cohorts 2 and 3

```{r}
pred <- ggpredict(m23, terms = "program_year")
...
pred <- ggpredict(m23, terms = c("program_year", "domain_type"))
...
preds <- ggpredict(m23, terms = c("program_year", "domain_type"))
pred_df <- as.data.frame(preds)

# 3️⃣ Plot
ggplot() +
  # Individual student trajectories (gray)
  geom_line(data = df_below_measures23,
...
Maybe change this to just use the df_below_measures dataset. might be cleaner? also- MMGI book filters 1.9 vs 2\>
